...
export default function SignInForm() {
    ...
    const handleIdentifierSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (result.status === 'needs_first_factor') {
                ...
                const hasEmailCode = result.supportedFirstFactors?.some(
            ...
            if (err && typeof err === 'object' && 'errors' in err) {
                ...
                const errorMessage = clerkErrors[0]?.message || 'Invalid email address'
    ...
    const handlePasswordSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (err && typeof err === 'object' && 'errors' in err) {
                const clerkErrors = err.errors as Array<{ code: string, message: string }>
                const errorMessage = clerkErrors[0]?.message || 'Invalid password'
        ...
        <div className={cn("flex flex-col gap-6")}>
            ...
            <form onSubmit={handleIdentifierSubmit} className="grid gap-6">
                ...
                <div className="space-y-2">
                    ...
                    />
                    {errors.identifier && (
                        <div className="text-sm text-destructive">{errors.identifier}</div>
                    )}
                </div>
                <Button type="submit" disabled={isLoading} className="w-full">
                    {isLoading ? (
