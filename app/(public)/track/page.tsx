...
const ORDER_STATUSES = [
  { status: 'pending', label: 'Order Placed', icon: ShoppingBag, color: 'bg-blue-500' },
  { status: 'in_progress', label: 'In Progress', icon: Loader2, color: 'bg-amber-500' },
  { status: 'ready_for_pickup', label: 'Ready', icon: Package, color: 'bg-emerald-500' },
  { status: 'delivered', label: 'Delivered', icon: Truck, color: 'bg-purple-500' },
  { status: 'completed', label: 'Completed', icon: CheckCircle2, color: 'bg-green-600' },
  { status: 'cancelled', label: 'Cancelled', icon: AlertCircle, color: 'bg-red-500' },
] as const;
...
export default function TrackPage() {
  ...
  const sendWhatsAppContact = () => {
    const message = encodeURIComponent(`Hi, I have a question about my WashLab order ${order?.orderNumber || ''}.`);
      ...
      <div className="container max-w-4xl mx-auto px-4 pt-24 pb-16">
        ...
        <div className="text-center mb-8 sm:mb-12">
          ...
          <p className="text-muted-foreground text-sm sm:text-base max-w-xl mx-auto">
            Enter your order number to check the real-time status of your laundry
          </p>
        </div>
          ...
          <CardContent className="p-6">
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Button 
                ...
                disabled={!searchQuery.trim() || (order === undefined && orderNumber !== null)}
                size="lg"
                className="h-12 px-8"
              >
        ...
        {order && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <Card className="shadow-xl border-2 overflow-hidden">
              <div className={`h-2 w-full ${isCancelled ? 'bg-destructive' : isCompleted ? 'bg-green-600' : isReady ? 'bg-emerald-500' : 'bg-primary'}`} />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium">₵{order.finalPrice.toFixed(2)}</p>
                    </div>
                  </div>
                </div>
              </CardContent>
                ...
                <div className="relative">
                  ...
                  <div className="space-y-6">
                    {ORDER_STATUSES.map((statusInfo, index) => {
                      ...
                      return (
                        <div key={statusInfo.status} className="relative flex gap-4">
                          {/* Timeline Line */}
                          {index < ORDER_STATUSES.length - 1 && (
                            <div className="absolute left-6 top-12 w-0.5 h-full">
                              <div 
                                className={`w-full transition-all duration-500 ${
                                  index < currentStatusIndex ? statusInfo.color : 'bg-muted'
                                }`}
                          ...
                          <div className={`relative z-10 flex items-center justify-center w-12 h-12 rounded-full transition-all duration-300 ${
                            ...
                            <StatusIcon className={`w-5 h-5 ${isCurrent ? 'animate-pulse' : ''}`} />
                          </div>
                          ...
                          <div className="flex-1 pt-1 pb-6">
                            ...
                            {isCurrent && (
                              <p className="text-sm text-muted-foreground">
                                ...
                                {order.status === 'in_progress' && 'Your laundry is being cleaned'}
                                {order.status === 'ready_for_pickup' && 'Your laundry is ready for pickup'}
              ...
              <Card className="shadow-lg">
                ...
                <CardContent className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-medium">{formatServiceType(order.serviceType)}</span>
                  </div>
                  {order.estimatedWeight && (
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-muted-foreground">Estimated Weight</span>
                      <span className="text-sm font-medium">{order.estimatedWeight} kg</span>
                    </div>
                  )}
                  {order.itemCount && (
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-muted-foreground">Items</span>
                      <span className="text-sm font-medium">{order.itemCount} pieces</span>
                    </div>
                  )}
                  {order.whitesSeparate && (
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-muted-foreground">Whites</span>
                      <span className="text-sm font-medium">Washed Separately</span>
                    </div>
                  )}
                  <Separator />
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">Base Price</span>
                    <span className="text-sm font-medium">₵{order.basePrice.toFixed(2)}</span>
                  </div>
                  {order.isDelivery && (
                    <>
                      <div className="flex justify-between items-center">
                  ...
                  <div className="flex justify-between items-center pt-2">
                    ...
                    <span className="text-lg font-bold text-primary">₵{order.finalPrice.toFixed(2)}</span>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="space-y-4">
                  {order.isDelivery ? (
                    <>
                      {order.deliveryAddress && (
                        <div>
                          ...
                          <p className="text-sm font-medium mt-1">{order.deliveryAddress}</p>
                        </div>
                      )}
                      {order.deliveryHall && (
                        <div>
                          <span className="text-xs text-muted-foreground">Hall</span>
                          <p className="text-sm font-medium mt-1">{order.deliveryHall}</p>
                        </div>
                      )}
                      {order.deliveryRoom && (
                        <div>
                          <span className="text-xs text-muted-foreground">Room</span>
                          <p className="text-sm font-medium mt-1">{order.deliveryRoom}</p>
                        </div>
                      )}
                      {order.deliveryPhoneNumber && (
                        <div>
                          <span className="text-xs text-muted-foreground">Contact</span>
                          <p className="text-sm font-medium mt-1">{order.deliveryPhoneNumber}</p>
                        </div>
                      )}
                    </>
                  ) : (
                    <div>
                      ...
                      <p className="text-sm font-medium mt-1">Branch Location</p>
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      <StatusBadge 
                        status={order.paymentStatus === 'paid' ? 'completed' : order.paymentStatus === 'pending' ? 'pending_dropoff' : 'completed'} 
                        size="sm"
                      />
                      {order.paymentMethod && (
                        <p className="text-xs text-muted-foreground mt-1">
              ...
              <Card className="shadow-xl border-2 border-emerald-200 dark:border-emerald-800 bg-emerald-50/50 dark:bg-emerald-950/20">
                <CardContent className="p-6">
                  <div className="text-center mb-6">
                    ...
                    <h3 className="text-xl font-bold mb-2">Your Laundry is Ready!</h3>
              ...
              <Card className="shadow-xl border-2 border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-950/20">
                <CardContent className="p-6 text-center">
                  ...
                  <p className="text-muted-foreground mb-4">
                    Please bring your clothes to your nearest WashLab branch and provide your order number
                  </p>
            ...
            <Card className="shadow-lg">
              ...
              <CardContent>
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <Button variant="outline" className="flex-1" size="lg" onClick={sendWhatsAppContact}>
        ...
        {!order && !orderNumber && (
          <Card className="shadow-lg">
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-xl font-semibold mb-2">Start Tracking Your Order</h3>
              <p className="text-muted-foreground max-w-md mx-auto">
                Enter your order number above to see the real-time status of your laundry order
              </p>
            </CardContent>
          </Card>
        )}
